...
/// <summary>
/// A generator for eight-dot braille character matrices. This generator constructs it's matrices using the entire 256
/// unicode character set for eight-dot braille as well as:
/// <list type="bullet">
...
/// </list>
/// </summary>
public class EightDotBrailleMatrixGenerator
{
    private readonly IBitmapAccessor _bitmapAccessor;

    /// <summary>
    /// Constructs a new <see cref="EightDotBrailleMatrixGenerator"/> instance.
    /// </summary>
    /// <param name="bitmapAccessor">A bitmap accessor for the bitmap from which to generate the character matrix.</param>
    public EightDotBrailleMatrixGenerator(IBitmapAccessor bitmapAccessor)
    {
        _bitmapAccessor = bitmapAccessor;
    }

    /// <summary>
    /// Generates an eight-dot braille character matrix from the bitmap provided to the constructor.
    /// </summary>
    /// <returns>A string containing the eight-dot braille character matrix.</returns>
    public string Generate()
    {
        var bitmapRows = _bitmapAccessor.Rows;
        var bitmapColumns = _bitmapAccessor.Columns;

        var matrixRows = (int)Math.Ceiling(bitmapRows / 4.0);
        ...
        for (var row = 0; row < bitmapRows; row++)
        {
            for (var column = 0; column < bitmapColumns; column++)
            {
                ...
                var byteRowIndex = row / 4;
                var byteColumnIndex = column / 2;
                var bitRowIndex = row % 4;
